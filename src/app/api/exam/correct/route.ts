...
import { correctImageWithAI } from '@/lib/gemini'
...
export async function POST(req: Request) {
  try {
    ...
    const { data: conversation, error: convError } = await supabase
      ...
      .insert({
        user_id: user.id,
        title: `تصحيح ${new Date().toLocaleDateString('ar-EG')}`,
        status: 'pending',
      })
      .select()
      .single()
    ...
    const aiResult = await correctImageWithAI(imageBase64, mimeType)
    ...
    if (aiResult.success) {
      ...
      try {
        ...
        if (!notifyResult.ok) console.error('Correction notify failed:', notifyResult.error)
      } catch (e) {
        console.error('Correction notify error:', e)
      }
      ...
      return NextResponse.json(
        {
          error: aiResult.error || 'فشل تصحيح الورقة',
          ai_status: 'failed',
        },
        { status: 500 }
      )
